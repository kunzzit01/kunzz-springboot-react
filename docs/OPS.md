...
### 7. 总库存总额与 live 差 1 分钱 —— 分组必须先 ROUND(...,2) 再 SUM（2026-09-17 实战，J2/J3）

**现象**：总库存-J2 老 live RM 28,377.72 vs 新系统 28,377.73；J3 同样差 0.01（类型卡金额两边完全一致）。

**根因**（不是数据问题，是舍入口径）：
- 老 live PHP：每个分组（产品+编号+规格+ROUND(price,2)）先 `round(净库存×显示单价, 2)`，**逐行舍入后再求和**
- 新系统原来：精确十进制求和（`SUM((in-out)×ROUND(price,2))`），只在最后四舍五入
- Kilo 类三位小数数量会产生半厘：J2 精确 28,377.725（A5 AWAGYU 0.293×554=162.322 / SALMON 0.61×51.3=31.293）→ live .72、新 .73；
  J3 精确 44,004.773（A5 AWAGYU 183.928 / SMOKED DUCK 6.17×21.5=132.655）→ live .78、新 .77

**修复**：`StockSummaryMapper.xml` summaryRows 的 total_price 改为
`ROUND(SUM((COALESCE(in_quantity,0)-COALESCE(out_quantity,0)) * ROUND(price,2)), 2)`（分组先舍入再 SUM）。

**规范（防再犯）**：
1. **不要把 total_price “优化”回精确求和**——老 live 就是逐行舍入口径；任何改动必须抓 live `stocklistapi.php?action=summary` JSON
   与本地 `/api/stock/summary` 逐行对账（方法见本节第 5 条排查三步）。
2. 此修复在**代码层**（mapper XML + jar），**导入新 dump 不需要重做**；但每次导库后仍要按第 5 条做总额对账，
   并确认运行的是 **2026-09-17 之后构建的 jar**（旧 jar 会重现 1 分钱差异）。
3. 实测基准（2026-09-17 dump 11 数据）：J1 31,907.19 / J2 28,377.72 / J3 44,004.78，
   与 live 逐行 275/218/317 行金额零差异（J3 4 行仅名称差异 = 本地已清洗 HTML 实体）。
