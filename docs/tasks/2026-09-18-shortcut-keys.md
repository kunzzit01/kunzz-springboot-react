...
## 用户要求的映射（**已按 2026-09-18 的追加要求修订**）

| 快捷键 | 进出货（StockInout）| 货品种类（StockProducts）|
|---|---|---|
| **Ctrl+S** | 保存**最上面一条待存行** | 保存**最上面一条待存行** |
| **Ctrl+Shift+S** | 批量保存 | 批量保存 |
| **Ctrl+Enter** | —（本来就没绑）| **去掉** |

> 第一版把 Ctrl+S 实现成"保存光标所在行"，用户实测后要求改成**不依赖光标**：
> 直接从上往下存，连按就一条一条存下去（截图里填了 4 条新增记录，希望连按 4 次存完）。
> 已改为"从上往下第一条待存行"。
>
> ⚠️ 两个页面的**待存行显示顺序正好相反**，所以"最上面一条"的判定也不同：
> - 进出货：已有记录（含编辑中的）在上、新增行在下 → 先编辑中的，再第一条新增行
> - 货品种类：新增行在上、已有记录在下 → 先第一条新增行，再编辑中的
...
### 追加验证：Ctrl+S 改为「从最上面开始」后（2026-09-18）

**进出货**：建 3 条新增行，进货数量都是 1，单价分别填 **11 / 12 / 13**（便于分辨哪条被存）：

| 第几次按 Ctrl+S | 结果 |
|---|---|
| 第 1 次 | 「成功保存 1 条记录」，待存行剩 **12、13** |
| 第 2 次 | 待存行剩 **13** |
| 第 3 次 | 待存行清空 |
| 第 4 次 | 「没有需要保存的记录」（提示一下，避免以为按键失灵）|

库里按 id 29292/29293/29294 依次写入 单价 11 / 12 / 13 —— 顺序正确。

**货品种类**（故意把光标放在**第二行**，验证不再依赖光标）：
光标所在行 = id 3，按 Ctrl+S 后**存掉的是最上面的 id 2**，id 3 仍在编辑态 ✓。
