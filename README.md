...
进出货页面右下角 🤖 聊天球：
- **库存问答**："apple sauce 还有多少"、"哪些货低于最低库存"（实时查库，不编数字）
- **单条进货/出货**："帮我进货 xx 2 件" → 生成草稿卡（自动补全编号/规格/最新单价）→ 确认后走原有验证流程
- **整单粘贴秒级出货**：直接贴分店订单（一行或多行、带不带 Date/Kitchen 表头都行），自动匹配货品、识别送往 J1/J2/J3，11 条订单约 1.5 秒出全部草稿

- 一键启动自动准备：Ollama + Qwen3-4B 模型（首次约 3.9GB，8 线程分块下载、断点续传，可选跳过；环境变量 `KUNZZ_SKIP_AI=1` 可永久跳过）
- 安全设计：AI/解析器只生成**草稿**，确认后才写库（走原有验证/HIFO/备注码流程），绝不直接改库
- 技术细节见 `docs/ai-assistant-progress.md`
