...
- **验证（本机生产副本 `kunzz_deploy` 的完整拷贝上实跑，两遍）**：
  ...
  ④ 第二遍执行残留仍为 0（幂等）。
- **上线动作**：脚本已在生产副本验证通过，**待在生产执行**（先 `mysqldump` 备份，再 `mysql u690174784_kunzz < fix_name_whitespace_20260925.sql`）；
  执行前第 0 节与执行后第 5 节的快照可直接对照。本机 `u690174784_kunzz`（9/17 旧副本）同样带双空格，需要时用同一条命令换库名即可。
