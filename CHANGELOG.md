...
### [2026-09-25-name-whitespace-cleanup] 总库存有货、出货却提示「可用库存 0」——货品名里夹了两个空格

- **现象（用户反馈）**：J3 出货 `SK 0045 / OZEKI JUNMAI DAIGINJO JUNDAN JIKOMI 700ML`（价格 RM324）被拒：
  「库存不足！可用库存: 0，请求出库: 1」，但总库存页该货品明明显示 `1.000 / RM 324.00`。
- **根因**：货品名里 `JIKOMI` 与 `700ML` 之间是**两个空格**（HEX `...4D49 2020 3730304D4C`，肉眼与单个空格一样）。
  货品名在本系统里同时是"货品身份"，而 2026-09-24 起写入端会把连续空白折叠成单个空格
  （`common/ProductName.java` 挂在 `StockInoutRequest` 等构造器上），**出货库存校验仍按 product_name 逐字节比对**
  （`availableStockBranch/availableStockCentral`）→ 规范化后的名字匹配不到 1 月录进来的历史行 → 可用库存算成 0。
  总库存页与价格下拉用的是"原样名字"，所以出现「看得到有货、出不了货」。这与 9/19 SURUME IKA 的裂行是同一类，
  区别是那次裂成两行（看得见），这次直接卡住出货（看不见）。
- **为什么只卡这一条**：全库扫描确认，名字带不可见空白的行共 4 个名字 146 行，**只有 OZEKI 这条净库存不为 0**；
  另外 3 个（`EGG  ( FREE )`、`HALF CUT  NORI`、`HALF CUT  NORI FUDO`）净库存为 0，属未爆的雷 ——
  一旦有进货（按规范化名字写入）就会当场裂成两行。9/24 那份 `fix_surume_ika_tab_20260924.sql` 是按货品写死的，没覆盖它们。
- **处理（只清洗数据，不动查询代码）**：查询端做空白兼容会让 `product_name` 索引失效、并掩盖数据漂移，
  而写入端已经规范化，所以按 9/24 既定口径清历史：新增 `fix_name_whitespace_20260925.sql`(仓库根目录)，
  覆盖全部 16 张表（中央/分店流水、分店台账、手机台账、货品主档、改价日志、总库存缓存、最低库存），
  4 个名字统一成单空格版；唯一键表（`jXstocklist_total`、`stock_minimum_settings`）先合并再改名；
  事务包裹、BINARY 精确匹配、不改 `updated_at`/`last_updated`、可重复执行。
- **验证（本机生产副本 `kunzz_deploy` 的完整拷贝上实跑，两遍）**：
  ① 修复前 16 张表异常名字 146 行（stockinout 65 / j1stockinout 13 / j1-3stockedit 11+9+28 / 手机台账 13 /
  主档 2 / 改价日志 1 / 三个缓存各 1 / 最低库存 1）→ 修复后 **全 0**，NBSP/全角空格/零宽空格等更狠的 HEX 扫描也全 0；
  ② 出货校验（规范化名 + RM324）从 **0.000 → 1.000**，价格下拉同样返回 `324.00 / 1.000`，总库存仍是**一行** `1.000 / RM 324.00`；
  ③ 各表行数 before=after（16/16 一致，没有行被吞）；四个系统净库存与总额**分文不变**
  （central 6047.689 / RM 90,821.77、J1 3341.766 / RM 32,040.00、J2 2758.740 / RM 29,788.27、J3 2815.748 / RM 47,172.75）；
  ④ 第二遍执行残留仍为 0（幂等）。
- **上线（2026-09-25 晚，已在生产执行并验证）**：服务器 `curl` 从 GitHub raw 取脚本到 `/opt/backups/`
  （sha256 `b734f5f3…`），`mysqldump` 备份后 `sudo mysql u690174784_kunzz < fix_name_whitespace_20260925.sql`；
  复核 `j3stockedit_data` 该行名字 **42 字节（双空格）→ 41 字节（单空格）**，**用户复测出货成功**。
- **过程中的三个坑（记下来免得再踩）**：
  ① 第一次「跑了脚本却没用」，是脚本根本没上传到服务器（`ls /opt/backups/...` → No such file），
     那份「自检第 5 节全 0」来自别的库。**最快的自检是第 0 节（执行前快照）：全 0 就说明库选错了**（正确的库这里必然非 0）；
  ② 中途一度把「用主档原样名字直接比台账」得到的 1.000 当成通过 —— 那是假阳性：后端会先把请求名规范化
     （`ProductName.normalize`）再比，两边都带双空格时当然"相等"。**验证必须复刻后端的确切口径**（脚本第 5 节 ③）；
  ③ 长 HEX 串在截图里会被 OCR 吃掉字符（把 42 字节看成 41）→ 核对用 `LENGTH()`，不要靠数空格。
- **复发防护**：脚本幂等，**以后每次重新导入旧 dump（如 `database/u690174784_kunzz.sql`）后重跑一遍**即可；
  日常不要手打货品名（从「货品种类」选），改名走「货品种类」编辑（会级联历史）。本机 `u690174784_kunzz`（9/17 旧副本）
  同样带双空格，需要时把库名换掉跑同一条命令。
- **防再犯指引（新增 `docs/PRODUCT_NAME_RULES.md`）**：给一线（一货一名一写法 / 不手打、不从 Excel·发票·微信复制 /
  出不了货先 F5 再报编号，别自己改名删记录）与管理员（**每月+每次导入 dump 后的例行体检 SQL**、
  异常名字的两种修法、出货被拦的三步排查〔名字字节→库在哪张表→请求的 system〕、验证方法论〔必须复刻后端规范化口径，
  端到端用真实 API 打一笔〕、可选加固）两套规矩。体检 SQL 已在事故数据与清洗后数据上双向验证：
  能抓出双空格 4 个名字与制表符版 `SURUME IKA<Tab>P`，清洗后的库返回空。
- **临时自救（不想等脚本时）**：在「货品种类」把该货品**点开保存一次**即可 —— 后端会规范化名字并触发改名级联
  （`ProductRenameService.cascade`，`WHERE product_name = BINARY 旧名`），台账历史一起改成单空格，随后即可正常出货。
...
- **现象（用户反馈）**：`/records` 总库存-中央 第 190、191 行都是 `PI 0034 / SURUME IKA P / Packet / RM 18.00`，
  库存分成两行 5.00 与 24.00（真实合计 29）。
- **根因（两层）**：
  1. **名字里夹了制表符**：中央流水 `stockinout_data` 里 184 行是 `'SURUME IKA<Tab>P'`（老 PHP 系统导进来的），
     2026-09-19 起新录入的是 `'SURUME IKA P'`（普通空格）。货品名在本系统里同时是"货品身份"——
     总库存 SQL `GROUP BY product_name`、Java 合并键（name+code+spec）、前端 `mergeSummaryItems`（按名 trim+小写）
     都按原样字符串分组，Tab 与空格分得开，于是同一条货品各自累计库存；
     而 HTML 渲染把 Tab 折叠成空格，页面上肉眼完全一样。
  2. **台账改名没有同步历史**：2026-09-19 09:45 有人在「货品种类」把台账 `stock_data` id=415 的名字
     从 Tab 版改成空格版（`updated_at = 2026-09-19 09:45:43`，与 9/17 的 dump 对照可证），
     流水里那 184 行仍是 Tab 版；57 分钟后第一笔新入库（入库 38）用的就是新名字 → 从此裂成两份库存。
- **数据修复**：`fix_surume_ika_tab_20260924.sql`（12 张表共 1053 行统一成空格版；事务包裹、可重复执行、
  不改 `updated_at`，避免把 1000+ 行历史刷成"今天改的"）。已在生产备份的完整副本上验证：
  中央 2 行 → 1 行 **29.000 / RM 522.00**，各系统总额分文不变（中央 RM 90,821.78）；
  顺带发现 J2、J3 也是双行（J2 9+42、J3 34+24），一并修好。**该脚本待在生产执行。**
- **代码改动（防复发）**：
  - 新增 `common/ProductName.normalize()`：HTML 实体解码 + 所有空白（Tab / 换行 / 不换行空格 / 全角空格 /
    零宽空格）折叠成单个半角空格。挂在写入端：`StockInoutRequest`、`StockDataRequest` 的紧凑构造器，
    `StockProductService` 新建/编辑/改价日志，`MobileStockService.normalizeName`。
  - 新增 `ProductRenameService.cascade()`：改名时把历史一起改（中央流水 / 分店流水 / 分店台账 / 手机台账 /
    手机版总库存缓存 / 最低库存 / 改价日志；带唯一键的表用"先合并数量 → 删旧名行 → 再改名"）。
    挂在「货品种类」编辑（这才是 9/19 那次的触发点）和「维护 → 重命名产品」两处 ——
    后者原来只改中央流水（JPA 全表扫描逐行 save），分店/台账/最低库存/改价日志全没跟上，改完反而多裂出几份。
- **验证（新代码真跑 + 真实接口）**：把测试副本的台账名字还原成 Tab 版（= 事故前状态），
  再用「货品种类」接口改名 → 116 行流水与 8 张关联表全部同步、Tab 残留 0，总库存合并成一行 29.00；
  进出货接口传带 Tab 的名字 → 入库即被规范化成空格；维护页改名 → 流水 + 台账 + 最低库存 + 改价日志一起改。
- **本机跑法**（09-23 那条记录说本机没 Maven、跑不了 `mvn package`）：Maven 在
  `C:\temp\mvn\apache-maven-3.9.9\bin\mvn.cmd`，用 `mvn -o compile` 编译、
  `mvn -o spring-boot:run -Dspring-boot.run.arguments="--server.port=8091 --spring.datasource.url=..."`
  可直接跑起来（不必打包 jar）。
...
### [2026-09-23-hire-claim-transfer] 后续调整：转交不再锁死 + 去掉联系方式后的提示词（用户试用后反馈）

- **用户反馈**：① 「任何工作人员都能去点击转交功能，不要锁死」；② 去掉邮箱/电话后面那串「（由他人跟进，不可联系）」。
- **改动 ①：转交对所有登录用户开放。** 原来 `JobService.transfer` 要求「你必须是当前处理人（或老板）」，
  非处理人点转交会被 409 拒绝 —— 处理人休假/转岗时这条就被卡住了。现在移除了这个归属判断，
  任何能看到招聘列表的人都能把一条申请转给别人，谁转的、从谁转给谁都会写进跟进记录
  （`阿娃 转交 转交给 ELAINE（原处理人 SOH ZHI ZEEN）`）。
  前端同步：详情弹窗里「转交给…」下拉不再只在「这条归我」时出现，不是处理人也显示；「释放」仍只给本人（未改动）。
- **保留的三条护栏**（用户没提要放开，且与「防止两个人重复联系」直接相关）：
  转交目标仍必须是 HR/老板（`account_type in (hr,special)`）、不能转给自己、
  以及**编辑权仍只归处理人**（非处理人改状态/备注仍 409）。
  另外新增一条：选中的就是当前处理人 → 直接提示「该申请已经由 X 处理」，不再写一条
  「转交给 X（原处理人 X）」的荒唐记录（这是上一版实测时发现的）。
- **改动 ②：移除「（由他人跟进，不可联系）」文案。** 只去掉了这串后缀，联系方式对非处理人**仍然不可点**
  （保持 `contact-locked` 的淡化样式与列表里的 title 提示），没有改变「别人不能从这里直接联系应聘者」的行为。
- **验证（重建 jar + 重启本地服务后实测）**：ELAINE（非处理人）成功把阿娃名下的申请转交给 SOH；
  阿娃（已不是处理人）再转回给 ELAINE；三条护栏与编辑权 409 均仍然生效。
  浏览器里以非处理人身份登录，确认弹窗顶部确实出现「转交给…」下拉、没有「释放」、
  且邮箱行只显示 `lailai.wong@example.com`、电话行只显示 `+60 0165551234`（旧提示词已从产物中消失）。
...
- **需求（用户反馈）**：停用功能一并去掉
- **为什么可以直接去掉**：有了「删除 + 级联清除引用」之后，停用是多余的 —— 不想再用某个冰箱，
  真正要做的就是把它从货品上摘掉（删除时会问、并列出影响多少个货品），而不是留一个"看不见但还在"的分类
- **实现**：
  - 前端：面板里每块的「停用/启用」按钮移除；`api/index.ts` 的 `setFreezerCategoryActive` 移除；
    `useFreezerCategories` 不再按 `is_active` 过滤（**能出现在面板上就一定能选**），也不再需要 `includeInactive` 参数
  - 后端：`PUT /{id}/active` 接口与 `FreezerCategoryService.setActive()` 移除；
    `list()` 不再有 `includeInactive` 参数（全部返回）；`create()` 里"同名已停用项则恢复启用"的分支随之简化
  - 面板提示文案同步改为「数字 = 有多少货品在用；🗑 = 删除」
- **数据**：`freezer_categories.is_active` 列保留（无害的遗留列，值恒为 1），未做 drop —— 删列是不可逆操作，
  没必要为一个已经没人读的字段去动表结构
- **移除前核对**：库里 18 个分类全部 `is_active=1`，没有任何分类处于停用状态，
  所以去掉这个功能不会让谁"消失且救不回来"
- **验证**：面板 18 个分类块 / 18 个拖拽手柄、面板内已无「停用/启用」按钮（只剩 🗑 图标按钮）；
  `GET /api/stock/freezer-categories` 去掉 `all` 参数后仍 200、返回 18 项且顺序不变；`PUT /{id}/active` 已 404；
  前端构建通过、后端重新打包并重启、无 JS 报错

### 8. 冰箱分类排序改为拖拽（原「↑↓ 点一下挪一格」太鸡肋）

- **需求（用户反馈）**：调整冰箱分类顺序要用拖拽，别用点上下箭头 —— 18 个分类里把最后一个挪到第一位要点 17 次
- **实现**：
  - 每个分类块左侧加 ⠿ 拖拽手柄，按住拖动；**拖动过程中本地顺序实时跟手**（被拖的块抬起来：琥珀描边 + 阴影 + 轻微放大，
    其余块即时让位，序号也实时重排），**松手才提交**
  - 只以手柄为拖动起点 —— 分类块里有可编辑的名字输入框，整块可拖会和选中文字/输入框抢焦点
  - **踩坑（重要）**：一开始用 `setPointerCapture` 把 move/up 绑在手柄上，实测拖到一半就断、松手也不提交。
    原因是拖动中列表会重排、手柄所在的 DOM 节点被移走，Chrome 这时会释放指针捕获。
    改为**拖动期间把 `pointermove/pointerup/pointercancel` 挂到 `window` 上**（拖动状态放 ref，
    监听只在开始拖时注册一次）后正常
  - 4px 位移阈值：单纯点一下手柄不算拖动，不会误改顺序
  - 拖动期间 `document.body.userSelect = 'none'`，否则鼠标划过其它分类的名字会选中一片文字
  - 手柄加 `touch-action: none`：否则触屏上拖动会被当成页面滚动
  - 用 Pointer 事件而不是 HTML5 drag：鼠标和触屏都支持，也没有原生拖影的锯齿感
  - 键盘兜底：手柄聚焦后 ←→ 挪一格（不占界面，给键盘用户留条路）
  - 「↑↓」两个小按钮已移除；底部提示同步改为「拖动手柄可以调整位置」
  - 后端零改动：`PUT /reorder` 本来就收有序 id 数组
- **验证（puppeteer 真拖，测数据已还原）**：
  - 把第 1 个分类拖到末尾：拖动中序号实时跟手（第 17 位 → 第 18 位）、松手后刷新页面顺序仍然一致（**已落库**）
  - 无 JS 报错；拖拽期间截图确认被拖块高亮、其余块让位

### 7. 冰箱分类按钮重新设计 + 修掉全站按钮 hover「看不见字」的老问题

- **需求**：货品种类页新加的「冰箱分类」按钮不好看，hover 也要优化
- **根因（顺带挖出一个全站性问题）**：`styles/media.css` 里的 `.btn` / `.btn:hover` / `.btn-danger` 等
  **完全没有页面作用域**（其它页面都写成 `.<xx>-root .btn`），而 Vite 会把所有 CSS 打进同一个文件，
  于是这几条全局规则污染了全站按钮。它给 `.btn:hover` 刷上「橙底 `#f98500` + 橙色光晕 + 上浮 2px」，
  再叠加 `index.css` 的 `.btn-default:hover { color: #ff5c00 }`（两者优先级相同、后者只管文字色）
  → **橙字压橙底，hover 时文字几乎看不见**。`btn-default` 在菜单/货品备注/分店/回收站/招聘/媒体等页都在用，
  所以这是全站性问题，不只冰箱按钮
- **实现**：
  - **全站修复**（`index.css`）：`.btn-default:hover` 补上自带背景 + 同色系柔和光晕 + 上浮 1px。
    自带背景是关键 —— 与 media.css 的 `.btn:hover` 优先级相同，靠打包顺序取胜，从而不再被刷成橙底
  - **冰箱按钮专属设计**（`stockproducts.css`，作用域限定 `.sp-root`，不影响其它页面）：
    - 常态 = 淡奶油底 `#fff8ec` + 琥珀描边 `#f6d9a8` + 琥珀雪花图标 + 深棕字 `#92400e`。
      一眼看得出是可点按钮，又用「浅色描边」与相邻两个实底主操作（添加新记录=绿 / 保存所有数据=橙）区分开，不抢视线
    - hover = 暖色浅底 `#fff2d9` + 描边加深到 `#f99e00` + 文字加深到 `#7c3a06` + 柔和琥珀光晕 + 上浮 1px
      （位移量与页内其它按钮一致；不再用刺眼的橙色实底）
    - 新增 `is-open` 状态：面板打开时切实底琥珀 + 白字，等同「按下」，看得出当前面板开着
  - **面板内小按钮**统一为 `.fc-mini`（↑↓ / 停用启用 / 🗑 / ＋加入 / 收起）：
    刻意**不挂 `.btn` 类** —— `.sp-root .btn` 有 `border: none` 会把边框清掉，而这些小按钮靠边框才看得出可点；
    样式全部自带，不再依赖 index.css / media.css 的全局按钮规则。
    hover 统一为奶油底 + 琥珀描边；删除按钮仅在 hover 时转危险色；单字符按钮加 `min-width: 22px` 成方形，点击区域更大
  - 名字输入框 `.fc-name`：hover 提示可编辑、聚焦给琥珀描边，暗示「改这里就能重命名」
- **验证（puppeteer 实拍 + computed style）**：
  - 冰箱按钮三态 computed style 全部符合预期：常态奶油底/深棕字、hover 暖底/更深棕字带柔和光晕、
    is-open 实底琥珀/白字；截图确认三种状态都清晰可读
  - **全站回归检查**：菜单管理 / 货品备注 / 分店 / 回收站 四页的 btn-default，hover 由原来的
    「橙底 #f98500 + 橙字 #ff5c00」变为「浅底 #fff7f0 + 橙字」，亮度对比差 128（修复前接近 0）
- **遗留（未改，供参考）**：`media.css` 里那几条无作用域的 `.btn*` 规则仍在，本次只在 `index.css` 侧做了
  补全式修复。要彻底收口需给媒体/背景音乐/页面上传三个页面加容器类再限定作用域，风险较高，留待单独处理

### 6. 冰箱分类可改名 / 可增删 / 可调顺序（原先 20 个名字写死在前端）

- **需求**：冰箱分类原先固定为 20 个（K1-1…SBDI-2），要能自己输入改名，且不破坏现有结构
- **关键事实**：冰箱分类没有字典表，**名字本身就是数据** —— 只存在 `stock_data.freezer_category` 一个字段
  （varchar(50)，逗号分隔多选，如 `K1-6,S1-2`）。总库存 / 手机版出货页都是从这一列派生的，
  所以改这一列即全局生效，不需要动任何其它表。唯一的真实障碍是**顺序**：
  前端 `FREEZER_OPTIONS` 硬编码数组的下标就是走冰箱的拣货顺序（总库存排序也用它），
  名字一改就掉进「未知分类」被排到最后，打乱拣货顺序
- **实现**：
  - **新表 `freezer_categories`**（`add_new_tables.sql` 第 5 节，纯增量）：只存【名字 + sort_order + 是否启用】，
    种子 = 原 20 项、顺序照搬原数组 → 上线当天界面零变化。种子只在表为空时灌入
    （否则改过名之后再跑本文件会把旧名字插回来）。`stock_data` 表结构与数据格式完全没动
  - **后端** `FreezerCategory{Controller,Service,Repository}` + `FreezerCategory` 实体：
    `GET /api/stock/freezer-categories`（下拉选项）、`POST`（新增）、`PUT {id}/rename`（全局改名）、
    `PUT {id}/active`（停用/启用）、`PUT /reorder`（调顺序）、`DELETE`（仍被货品引用则拒绝，提示改用停用）
  - **改名是最需要小心的一步**（`FreezerCategoryService.rename`）：
    ① 先全量试算，任何一行会超长就整体报错取消 —— 绝不写一半；
    ② **token 级精确替换**（按逗号 split 后比对），不用 SQL `REPLACE` ——
    否则 `REPLACE(...,'K1-1',...)` 会把将来的 `K1-10` 一起改坏；替换后去重（可能撞上串里已有的另一个分类）；
    ③ 只更新 `freezer_category` 单列，不整行覆写；④ 超长（>50）时点名受影响的货品，提示换个短名字
  - **权限**：写操作要求货品种类页的「批准」权限（canApprove），与后端 `assertCanApprove` 一致；
    未配置过 `stock_inventory` 权限记录的老账号按现有约定默认放行
  - **前端**：新增 `utils/useFreezerCategories.ts` 收敛原先散落的硬编码（`FREEZER_OPTIONS`@StockProducts、
    `FIXED_AREAS`@MobileOut）—— 选项与顺序改为从接口取，接口不可用时回落同一份内置清单，页面照常可用；
    货品种类页「冰箱分类」多选下拉加了输入框（可筛选，输入新名字可直接建成新分类），
    工具条新增「冰箱分类」维护面板（改名 / ↑↓ 调顺序 / 停用 / 删除 / 新增，并显示每个分类被多少货品使用）；
    改名前弹确认框告知会影响多少个货品。总库存排序逻辑一行未改，只是顺序表来源换了
- **验证（真跑接口 + 数据对账，测试数据已全部还原）**：
  - 改名级联：`K1-6` → `TESTM` 命中 27 行（与使用量统计一致）；多值行 `K1-6,S1-2` → `TESTM,S1-2`
    （S1-2 分毫未动、顺序不乱）；旧名残留 0；改名前后总量一致
  - 边界：含逗号 / 超 50 字符 / 重名 / 空名 分别被拒；挂两个冰箱的货品（`K1-6,S1-2`）改成 46 字符名字时
    整体拒绝并点名「SOFT SHELL CRAB」，字典未被改动
  - 增删改查与排序：新增排到末尾、重复新增被拒、停用后默认列表不含它、调顺序后 `sort_order` 正确
  - `add_new_tables.sql` 连跑两遍幂等；字典 20 行顺序与原数组完全一致
- **补充（用户反馈后）：删除要真能删掉 + 改名不该打乱排序**
  - **删除支持级联清除引用**（`FreezerCategoryService.delete(id, force)`）：分类还有货品在用时，
    不带 `force` 会拒绝并提示改用停用（安全兜底）；带 `force=true` 则**删除的同时把这些货品上的该分类去掉**
    （多冰箱货品其余分类保持不变），前端弹确认框写明「会让 N 个货品失去这个分类」再执行
  - **修掉一个我引入的真问题**：`freezer_category` 上的 `ON UPDATE CURRENT_TIMESTAMP` 会把被改名货品的
    `updated_at` 顶成当前时间，导致货品种类页整批重排（已批准行按 `updated_at` 升序），
    进货默认单价（`ORDER BY updated_at DESC`）也可能换一条来源。
    现在 `updateFreezerOnly` 用 `updated_at = updated_at` 自赋值抑制自动更新 —— 分类改名属于全局维护，
    不改变货品本身的业务内容，不该动它的「最后修改时间」
  - 实测（只动自建测试分类，不碰真实数据）：不带 force 删除被拒 → 带 force 删除返回 `cleared:1` →
    货品 `K1-6,S1-2,ZZTEST` 精确还原为 `K1-6,S1-2`，且 `updated_at` 保持 `2026-04-07 12:58:33` 未变；
    0 引用时直接删、改名、调顺序均正常；测试分类残留 0
- **上线状态**：已上线。数据库建表灌种子（20 项 + 用户新建的 K1-1A）；前端已构建部署到 `backend/static`；
  后端已重新打包并重启（`mvn -DskipTests package` + `一键启动.bat`，注意 Windows 会锁 jar，必须先停后端）

### 5. 货品种类：多行同时改 + 批量保存（保一行不再冲掉其他行）+ 快捷键

- **需求**：货品种类页要像进出货页那样支持批量修改/批量保存——点其中一行的保存，不能影响其他行已改但没保存的内容；并补上适用的快捷键
- **根因（旧行为）**：`load()` 里无条件 `setEditing(new Set()) + setDrafts({})`。保存单行、批准、删行、刷新都会走 `load()`，
  于是点一行保存＝所有行未保存的修改一起消失（改了几行只能一次全存，中间不能单独存一行）
- **实现（`StockProducts.tsx` + `stockproducts.css`）**：
  - 草稿按行 id 保留：`load()` 不再清空，只丢弃「这一版数据里已经没有的行」（被别人删了 / 切到别的系统）；
    搜索过滤掉的行草稿也留着（清空关键字回来还在），避免边改边搜一下就把输入丢了。
    切系统显式传 `keepMissing=false` —— 否则跨系统草稿会被 `Ctrl+S` 按当前系统写回去，改了别家的货
  - 行内保存拆出 `doSaveEdit/doSaveNewRow`（返回失败原因），成功才摘掉那一行：`delete` 只针对该 id，其他编辑行的草稿原样保留
  - 批量保存「保存所有数据 (N)」逐条保存：单条失败只拦截该条并留在原位（草稿不丢，改完再按 Ctrl+S 重试），
    toast 汇总「已保存 N 条，M 条失败：原因」；旧版是过滤掉不合法的新行、且任一行抛错就整批中断
  - 新增行改用稳定 `_key`（`data-new-key`），删行后下标错位也不会摘错行
  - 批准后同步草稿里的 `approver`：否则这行随后一保存就把刚批准的记录又打回待批准（草稿存的是批准前的值）
  - 工具条新增「取消编辑 (N)」按钮（确认后一次性放弃全部未保存修改；新增行仍用各自的行内删除）
  - 快捷键（对齐进出货页）：**Ctrl+S** 保存全部、**Ctrl+Enter** 保存光标所在行、**Ctrl+A** 新增一行（表格内可连按，不抢输入框全选），
    工具条加键盘提示，按钮 title 同步标注
- **验证（浏览器实测，测试数据已还原）**：
  - 两行同时改供应商 → 点第 2 行「保存」→ 第 2 行入库；**第 3 行仍在编辑态且值没丢**（旧版这里会被清空）
  - 新增行填满后单独保存 → 行入库并从表单消失，同页另外两行编辑中的内容不受影响
  - `Ctrl+Enter` 保存光标所在行 ✅；`Ctrl+S` 一次保存 2 行（toast「已保存 2 条记录」）✅；`Ctrl+A` 加行 ✅；
    只有一条没填完的新行时 `Ctrl+S` 提示「保存失败：请填写完整的货品编号、名称、规格、类型、供应商」且不写库 ✅
  - 收尾：4 行供应商恢复原值（SANSUI/SENRI/WIN FAR）、测试行 811 已删除，货品总数回到 610
- **顺带发现（本次未改）**：总览里 `system_assign` 为空的新行保存后不会出现在列表（`load()` 的总览过滤要求系统分配 ≥ 1 间），
  属既有行为；若要改可让空分配的行也显示或新增时强制选择
- **踩坑（17:30 被并发会话整文件覆盖，已重贴）**：另一个会话在同一份 `StockProducts.tsx` 上做冰箱分类面板，
  整文件重写时把本节改动连同快捷键一起覆盖掉了（部署包 17:31 重建后页面上就没有快捷键了）。
  已在对方最新版本上逐处重贴（冰箱分类面板与工具条按钮保持原样），并重新构建部署、浏览器复测通过。
  **副本备份**：`.zcode/backup-batchsave/`（`StockProducts.tsx` 整文件 + `.vs-HEAD.patch` + `stockproducts.css`）——
  若再次被覆盖，直接从这里取回重贴即可。另：这份改动尚未提交，并发会话仍可能覆盖，建议尽快 commit 固化。

### 10. 进出货：备注编号清单高亮当前选中项

- **反馈**：重新打开清单时看不出自己选的是哪一条
- **处理**：清单项按当前编号做 **active 高亮**（浅橙底 + 左侧橙条 + 加粗），值匹配按「编号后缀」比对
  （输入框只存 `195`，清单项是 `SA-195`，两边归一后比对）；鼠标移开后仍保持高亮
- **验证（浏览器实测）**：选 SA-195 后重新打开 → 该项带 `active`、底色 `rgb(255,244,224)`、字体加粗 ✅

### 9. 进出货：点编号框弹出清单后不再「自动关闭」

- **现象**：点编号框弹出清单，一松手就自己关了（挂不住）
- **根因**：编号框用 `mousedown` 打开清单，紧接着同一次点击的 `click` 事件冒泡到 document，
  命中「点选择器外部就关闭」的逻辑，于是刚开就被关掉
- **修复**：外部点击关闭的判断里排除**编号框自身**（加 `.remark-code-input` 标记）——
  开关由编号框自己负责；点其它地方照旧关闭
- **验证（浏览器实测）**：真实鼠标「按下→抬起」后面板保持打开、11 项在列；点选 SA-195 → 回填 `SA` + `195`，清单收起

### 8. 进出货：备注编号去掉箭头按钮（改为点编号框弹出）+ 修编号框被挤成 8px

- **反馈①**：选了编号后表格里看不到 —— 实际是**编号输入框被挤到 8px 宽**（上次加箭头按钮时改成 `flex:1; minWidth:0`，同排再插按钮就压没了），数值其实写进去了只是看不见
  - 修复：编号框改 `flex: 1 1 44px; minWidth: 30px`（新增行 + 编辑行两处）
- **反馈②**：既然看得见了，箭头按钮影响美观 → **去掉箭头**
  - 改为**点编号输入框本身**弹出「在库编号 + 剩余量」清单（`onMouseDown`，触屏用 `onPointerDown` 兜底）；
    不加 `preventDefault`，所以仍可正常聚焦、手动输入；手动输入时自动收起清单
- **验证（浏览器实测）**：箭头已不存在；编号框 30px；点框弹清单（SA-194…）；选 SA-195 → 回填 `SA` + `195`，清单自动收起

### 7. 进出货：备注编号选择器点选后一定能回填（双通道 + 一次触发守卫）

- **现象**：清单里点了某个编号（如 SA-194），表格里的备注编号没变
- **处理**：清单项同时挂 `onMouseDown` 与 `onClick` 两条路径（浏览器/鼠标操作差异下任一条都能触发），
  用一次性守卫保证一次选择只回填一次；每次重新打开选择器时重置守卫
- **验证（浏览器实测 3 种点击方式）**：元素点击 / 鼠标坐标点击 / 按下停顿再抬起 → 均回填 `SA` + `196`，无重复、无漏填

### 6. 进出货：HIFO 拆行改为「保留用户填的数量 + 一键拆分」（用户反馈填 80 只留 0.84）

- **现象**：出货填 80，行里只剩最高价那层的 0.84，用户填的数量被改写
- **改为**：
  - **用户填多少就保留多少**（80 就是 80），不再擅自把原行改写成单层数量
  - 当这一单跨多个价格层时，单价列出现 **「HIFO 拆 N 行」按钮**（如「HIFO 拆 4 行」），点它才拆：
    第一层留在原行、其余逐层插到其后（0.84@51.70 + 7.70@50.70 + 23.41@50.55 + 48.05@45.50）
  - 出货量/货品/编号变化时，之前拆出的行会自动作废、提示重新计算
- **顺带修**：只点货品名（没点具体编号）时，同名多供应商里如果落到的是一家**没库存**的，
  现在会自动改用「有库存的那一家」——原行为固定取台账第一行，可能正好没货，出货就只能空着
- **验证（浏览器实测）**：先填 80 → 选 SALMON (TAC) → 数量仍为 80、出现「HIFO 拆 4 行」→ 点击后拆出 4 行（价格 51.70/50.70/50.55/45.50）✅

### 5. 进出货：HIFO 自动拆行补齐「先填出货量、后选货品」的场景（用户反馈 SALMON 出 80）

- **现象**：出货 SALMON 填 80，只按最高价那一层扣（51.70 只有 0.84），扣不了其余的量
- **根因**：HIFO 自动拆行只写在 `handleOutQty`（改「出货」输入框时触发）。用户先填出货量 80、再去选货品时，
  走的是 `onPickProduct` 的出货分支——那里只加载价格批次、**不拆行**；而且事件闭包里的 `newRows` 是旧值（读到的 outQty 为空），
  所以拆行条件永远不成立
- **修复**：
  - 拆行逻辑抽成 `applyHifoSplit(key, outQty, productName, codeNumber)`，`handleOutQty` / `onPickProduct` / `onPickCode` 三处共用
  - 新增 `newRowsRef`（最新行快照）+ `pickRowOutQty()`（优先读该行「出货」输入框的 DOM 值）：绕开 React 状态滞后
  - 出货输入框加 `out-qty` class、行加 `data-key`，供上面的 DOM 读取定位
- **验证（浏览器实测，SALMON 出 80）**：
  - **先填 80 再选货品** → 拆 4 行：0.84@51.70 + 7.70@50.70 + 23.41@50.55 + 48.05@45.50（编号 FRI 0034）✅
  - **先选货品再填 80**（原有路径，防退化）→ 同样拆 4 行 ✅
  - 未选编号（按货品全量批次）→ 同样正确拆行 ✅

### 4. 进出货出货：备注编号可选（列出在库编号 + 各自剩余量），不必再跑「货品备注」页

- **需求**：出货填备注编号时，直接给出可用的编号和数量/Kilo 让用户选，省掉先去货品备注页查看/核对再回来手输的步骤
- **实现**：
  - 后端 `GET /api/stock/remark-code-options?productName=`：按货品返回在库备注编号 + 剩余量 + 单位
    （`StockEditMapper.remarkCodeOptions`：净库存 > 0，与既有的 `remark-codes` 校验查询同口径；单位取该编号最近一条流水的 specification；
    `StockEditService` 做自然排序 SA-9 < SA-10）
  - 前端（`StockInout.tsx` + `stockinout.css`）：出货行（勾了货品备注且有出货数量）的备注编号框右侧多一个下拉钮，
    点开列出 `SA-194  剩余 22.740 Kilo` 形式的清单，点一条即回填前缀与编号（新增行拆成前缀/编号两个输入框、编辑行回填完整编号）
- **不改动的**：编号仍可手输、保存校验（编号必须在库）与后端写库口径完全不变；没有备注编号的货品该按钮不出现（点开也会提示"该货品当前没有在库备注编号"）
...
### [2026-09-17-anti-collision] 多任务并行防撞：规则 + 钩子 + 更新脚本闸门

- **背景**：同一台机器同时跑多个 task，各自改代码、各自推 `main`，出现互相覆盖。查明两条真实机制：
  ① 多 task 共用**同一个 git 索引**，`git add -A` 会把别人的改动一起打包进自己的提交；
  ② `git-update.ps1` 的 `reset --hard origin/main` 会抹掉整个工作区，**并把本地已提交但未推送的
  提交从分支上摘掉**（只剩 reflog 能捞）——看起来就像"代码被别的 task 覆盖了"
- **根因（已实测定性）**：真正吃掉代码的是**并发会话整文件重写**。17:30 一个会话整文件重写
  `StockProducts.tsx`，把另一个会话尚未提交的批量保存 + 快捷键一起覆盖掉（见上面 #5 那条记录）。
  这类覆盖**没有冲突提示、没有备份、git 也帮不了忙**——文件内容是"合法"的，只是少了别人的改动
- **规则**（`AGENTS.md`，五条铁律）：显式路径暂存、只快进推送、冲突涉及白名单外文件就停下报告、
  一个文件同一时刻只属于一个 task、**并行期间禁止运行 `Git更新.bat` / `git-update.ps1`**。
  新增 1.1 节「整文件重写 = 最危险的覆盖」：动手前用 `git status --porcelain -- <文件>` 看它脏不脏
  （脏 = 已有人在改，此刻不属于你）、禁止整文件重写不属于自己的文件、
  大范围重写前先整份复制到 `.zcode/backup-<任务ID>/`
- **强制措施**（`.githooks/`，用 `git config core.hooksPath .githooks` 启用）：`pre-commit` 按
  `docs/tasks/<任务ID>.md` 的白名单拦下白名单外的暂存文件并告知 `git restore --staged`；
  `pre-push` 拦强推 / 非快进推送 main。四场景实测：非快进拒绝、快进放行、
  自己的 `task/*` 分支强推放行、删除 main 拒绝
- **更新脚本闸门**：`git-update.ps1` 在重置前判断本地 `main` 是否领先 `origin/main`，领先则中止并
  提示 `git pull --rebase`；检查位于**任何 reset 之前**，中止时索引与工作区均未被改动。
  三态实测：领先 1 个提交→中止；本地 == origin→放行；落后于 origin（部署机正常更新）→放行
- **顺带修复**：本地改动备份的排除规则由 `*.sql` 收窄为 `database/*.sql`——原先
  `add_new_tables.sql` 等手写 SQL 的改动不会被备份，现已照常备份（仍排除 22MB 数据包）
- **加固**：`.gitattributes` 给 `CHANGELOG.md` 开 `merge=union`（冲突时两边日志都保留，不再整篇被
  一侧覆盖）；钩子脚本固定 LF 换行，否则 Git for Windows 下会报 `cannot run`
- **排查副产物（与本仓库所有操作相关）**：`git status` 在本仓库会大量误报——
  `backend/static/**` 等约 113 个文件是 **OneDrive「仅在线」占位文件**（ReparsePoint），
  git 每次 stat 都对不上索引缓存，于是永远报「已修改」，**实际内容与 HEAD 逐字节相同**
  （`git hash-object` 与 `git rev-parse HEAD:<文件>` 比对可自证）。
  实测 `git status` 报 125 个、`git diff` 只有 12 个真实改动，**差 10 倍**。
  所以本仓库里 `git add -A` 格外危险（会强制 OneDrive 下载一百多个占位文件并搅乱索引）。
  判断真实改动一律看 `git diff`；根治办法：把仓库移出 OneDrive，或对该文件夹选「始终保留在此设备上」
