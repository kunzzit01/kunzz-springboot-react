...
import {
  getPriceCompare, getRestaurants, saveRestaurant, deleteRestaurant,
  getSuppliers, createSupplier, deleteSupplier,
  saveRestaurantFood, updateRestaurantFood, deleteRestaurantFood,
...
interface EntityItem { id: number; label: string; sub?: string }

export default function Price() {
  const navigate = useNavigate()
  const [mode, setMode] = useState<'restaurant' | 'supplier'>('restaurant')
  ...
  const [entityCn, setEntityCn] = useState('')
  const [entityEn, setEntityEn] = useState('')

  // 批量新增行（行数 modal → 表格底部可编辑行 → 批量保存）
  ...
  // ---------- 数据加载 ----------
  const load = (m: 'restaurant' | 'supplier') => {
    setLoading(true)
    return getPriceCompare(m)
      .then((d) => {
        setData(d)
  ...
  useEffect(() => { load(mode) }, [mode]) // eslint-disable-line
  ...
  // 已存在实体列表（新增 modal 内展示）
  useEffect(() => {
    if (!showAddEntity) return
    if (mode === 'restaurant') {
      getRestaurants().then((rs) =>
        setEntities(rs.map((r) => ({ id: r.id, label: r.nameCn || r.nameEn || r.name || '#' + r.id, sub: r.nameEn || '' }))),
      ).catch(() => {})
    } else {
  ...
  const saveCell = async (colId: string, cell: { id: number } | undefined, payload: Record<string, unknown>) => {
    if (mode === 'restaurant') {
      if (cell) await updateRestaurantFood(cell.id, payload)
      else await saveRestaurantFood(Number(colId), payload)
    } else {
  ...
  const deleteCell = async (cellId: number) => {
    if (mode === 'restaurant') await deleteRestaurantFood(cellId)
    else await deleteSupplierMaterial(cellId)
  }
  const nameField = mode === 'restaurant' ? 'foodName' : 'materialName'
  const typeField = mode === 'restaurant' ? 'foodType' : 'materialType'

  // ---------- 新增餐馆 / 批发商 ----------
  const saveEntity = async () => {
    try {
      if (mode === 'restaurant') {
        const cn = entityCn.trim().toUpperCase()
        const en = entityEn.trim().toUpperCase()
        if (!cn) { showMsg('请填写中文名称', 'error'); return }
        await saveRestaurant({ nameCn: cn, nameEn: en || cn, name: cn })
      } else {
        const n = (entityCn || entityEn).trim().toUpperCase()
        if (!n) { showMsg('请填写批发商名称', 'error'); return }
        await createSupplier(n)
      }
      showMsg(mode === 'restaurant' ? '餐馆已新增' : '批发商已新增')
      setShowAddEntity(false); setEntityCn(''); setEntityEn('')
      load(mode)
    } catch { showMsg('保存失败', 'error') }
  }
  const removeEntity = async (id: number) => {
    if (!window.confirm('确定删除该' + (mode === 'restaurant' ? '餐馆' : '批发商') + '及其全部价格记录？')) return
    try {
      if (mode === 'restaurant') await deleteRestaurant(id)
    ...
    try {
      ...
      for (const r of rows) {
        ...
        for (const col of visibleCols) {
          ...
          if (!v) continue
          await saveCell(String(col.id), undefined, { [nameField]: name, [typeField]: r.type || null, price: Number(v) })
        }
      }
  ...
  const saveEdit = async (row: PriceCompareRow) => {
    try {
      for (const col of visibleCols) {
        ...
        await saveCell(String(col.id), cell, { [nameField]: row.name, [typeField]: row.type || null, price: Number(v) })
      }
      showMsg('已保存')
      setEditRow(null)
      load(mode)
    ...
    <div className="price-root">
      <div className="pr-header">
        ...
        <div className="pr-header-right">
          {/* 对比模式选择器（点击展开：餐厅对比 / 批发商对比） */}
          <div className="restaurant-selector">
            <button className="selector-button" onClick={(e) => { e.stopPropagation(); setModeOpen(!modeOpen) }}>
              <span>{mode === 'restaurant' ? '餐厅对比' : '批发商对比'}</span>
              <i className="fas fa-chevron-down"></i>
            </button>
            {modeOpen && (
              <div className="selector-dropdown show">
                <div className={'dropdown-item' + (mode === 'restaurant' ? ' active' : '')}
                  onClick={(e) => { e.stopPropagation(); setMode('restaurant'); setModeOpen(false) }}>餐厅对比</div>
                <div className={'dropdown-item' + (mode === 'supplier' ? ' active' : '')}
                  onClick={(e) => { e.stopPropagation(); setModeOpen(false); navigate('/suppliers') }}>批发商对比</div>
              </div>
            )}
          </div>
          {/* 餐厅选择器（总览 + 各餐厅，展开式设计） */}
          ...
          <div className="filter-left">
            ...
            <div className="filter-group" style={{ flex: '0 0 200px' }}>
              ...
              <div className="smart-search">
                ...
                <input value={kw} onChange={(e) => setKw(e.target.value)} placeholder="输入货品名称" />
              </div>
            </div>
          </div>
          <div className="filter-right">
            <button className="btn btn-success" onClick={() => setShowAddEntity(true)}>
              <i className="fas fa-store"></i> {mode === 'restaurant' ? '新增餐馆' : '新增批发商'}
            </button>
      ...
      <div className="table-container">
        <div className="table-scroll-container">
          <table className="price-table">
            ...
            <tbody>
              {loading && (
                <tr><td colSpan={colCount} className="table-loading">加载中...</td></tr>
              )}
              {!loading && filtered.length === 0 && (
                <tr><td colSpan={colCount} className="table-loading">暂无数据</td></tr>
              )}
                ...
                return (
                  <tr key={row.name} className={isEditing ? 'editing-row' : isHighlight ? 'highlight-flash' : ''}>
                    ...
                    <td className="cell-type">
                      <span className="cell-text">{row.type || '-'}</span>
                    </td>
                    {visibleCols.map((c) => {
              ...
              {newRows.map((r, ri) => (
                <tr key={'new' + ri} className="new-row">
                  ...
                  <td className="cell-name">
                    <input className="table-input" placeholder="货品名称" value={r.name}
        ...
        <div className="stats-bar">
          <span>共 <span className="stat-value">{filtered.length}</span> 项货品 · <span className="stat-value">{visibleCols.length}</span> 个{mode === 'restaurant' ? '餐厅' : '批发商'}</span>
          <span>
            ...
            <span style={{ display: 'inline-block', width: 12, height: 12, background: '#a4c2f4', border: '1px solid #7f9fd6', margin: '0 4px 0 14px' }}></span>{mode === 'restaurant' ? '餐厅' : '批发商'}独有
          </span>
        </div>
      </div>
        ...
        <div className="modal-overlay" onClick={(e) => { if (e.target === e.currentTarget) setShowAddEntity(false) }}>
          <div className="modal-content" style={{ maxWidth: 560 }}>
            <div className="modal-header">
              <h3 className="modal-title">{mode === 'restaurant' ? '新增餐馆' : '新增批发商'}</h3>
              ...
              {mode === 'restaurant' ? (
                <>
                  <div className="form-group">
                    <label>中文名称 *</label>
                    <input className="form-input" placeholder="输入中文名称..." value={entityCn}
                      onChange={(e) => setEntityCn(e.target.value.toUpperCase())} style={{ textTransform: 'uppercase' }} />
                  </div>
                  <div className="form-group">
                    <label>英文名称 *</label>
                    <input className="form-input" placeholder="Enter English name..." value={entityEn}
                      onChange={(e) => setEntityEn(e.target.value.toUpperCase())} style={{ textTransform: 'uppercase' }} />
                  </div>
                </>
              ) : (
                <div className="form-group full-width">
                  <label>批发商名称 *</label>
                  <input className="form-input" placeholder="输入批发商名称..." value={entityCn}
                    onChange={(e) => setEntityCn(e.target.value.toUpperCase())} style={{ textTransform: 'uppercase' }} />
                </div>
              )}
            </div>
            <div className="existing-restaurants">
              <div className="section-title">已存在{mode === 'restaurant' ? '餐馆' : '批发商'}</div>
                ...
                {entities.map((e) => (
                  <div key={e.id} className="restaurant-item">
                    <div className="restaurant-names">
                      ...
                      {e.sub && <span className="restaurant-name-en">{e.sub}</span>}
                    </div>
